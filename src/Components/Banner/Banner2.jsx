        ...
        <div className='container'>
            <div  style={{backgroundColor:data.bgColor}}  className='grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-white rounded-3xl'>
                ...
                <div className='h-full flex items-center'>
                    <img src="smartwatch2-removebg-preview.png" alt="" className='scale-125 w-[250px] md:w-[340px] mx-auto drop-shadow-2xl object-cover' />
                </div>
                {/* third col */}
                <div className='flex  flex-col justify-center gap-4 p-6 sm:p-8'>
                    <p className='font-bold text-xl'>{data.title2}</p>
                    <p className='text-3xl sm:text-5xl font-bold'>{data.title3}</p>
                    <p className='text-sm font-semibold'>{data.title4}</p>
                    <div><button className='bg-white text-brandGreen py-2 px-4 rounded-full'>Shop Now</button></div>
                </div>
            </div>
        </div>
    </div>
